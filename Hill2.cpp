...
void xemmang(int a[][500],int x){
    printf("\n");
    for (int i=0;i<x;++i){
        for (int j=0;j<x;++j) printf(" %5d ",a[i][j]);
        printf("\n");
    }
}
void docdl(){
...
int timsonghichdao(int a){
    int r,g,y,y0=0,y1=1,tga=a,m=26;
    while (a>0){
        r=m%a;
        if (r==0) break;
        q=m/a;y=y0-y1*q;m=a;a=r;y0=y1;y1=y;
    }
    if(a>1) return -1; else if(y>0) return y; else return y+26;
}
void timmatrankhoagm(int khoand[][50]){
    ...
    if (khoand[0][1]<0) khoa[0][1]+=26;
    khoand[1][0]=(detnd*khoa[1][0])%26;
    if (khoand[1][0]<0) khoa[1][0]+=26;
}
char *giaima(){
    char s[100];
    ...
    while (i<t){
        int kq[50],vecto[50];
        for(int j=0;j>n;++j) vecto[i]=xaugm[i+j]-65;
...
int main(){
    ...
    char mh[100],gm[100];
    strcpy(mh,mahoa());
    if (n==2) strcpy (gm,giaima()); else gm[0]=NULL;
    printf(".%s. ma hoa duoc .%s.",xaumh,mh);
    printf(".%s. giai ma duoc .%s.",xaugm,gm);
    fprintf(u,".%s. ma hoa duoc .%s.",xaumh,mh);
    fprintf(u,".%s. giai ma duoc .%s.",xaugm,gm);
    printf("\n HOAN TAT Vao tep output.txt xem ket qua ");
    fclose(u);
    getch();
}